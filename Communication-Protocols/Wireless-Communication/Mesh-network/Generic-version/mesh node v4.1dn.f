...
\       Mesh node 4.x:                      2320 bytes (2666) (3364)
\       Total:                              3784 bytes (4186) (5034)
\       Free with use of small tools:       3728 bytes (3316) (1930)
\       Free RAM:                            140 bytes  (130)   (90)
...
: EXEC      ( c exec-table -- )
    swap >r                 \ Save c
    begin
    dup @ dup -1 <> while   \ Get command token, ready if -1
        >r  2 cells +       \ No, to next entry
    r> r@ = until           \ Command token found?
    cell- @ execute         \ Yes, fetch & execute xt
    else  drop              \ Not found, remove table address
    then  rdrop ;           \ and command token

: LOC ( node a -- bit-mask adr ) \ Bit location in byte-addr
    >r  8 /mod  #map 1- and \ Byte number
    r> + >r                 \ Build byte address
    1 swap lshift  r> ;     \ Bit mask byte-address
\ : LOC ( node a -- bit-mask adr ) \ Bit location in byte-addr
\   over 3 rshift           \ Byte number
\   #map 1- and  + >r       \ Keep in range, add to address, save
\   1 swap 7 and lshift r> ; \ Lowest 3-bits to bit mask
...
: GET       ( -- )
    key? if                                 \ Key pressed?
        key dup 1B = if  abort  then        \ Yes, quit on escape char!
...
: NODE     ( -- )
    startnode  troff  0 to len
    begin  begin  handler? until  get  again ;


spi-setup                   \ Activate SPI interface
55 1  3 1  FF setrf  node   \ Default RF value & node number, etc.
